...
const panes: ReadonlyArray<PaneConfig> = [
  ...
  {
    title: "Round End Rewards",
    shortcut: `r`,
    path: "/settings/rewards",
    Pane: (props: LobbiesPaneProps) => <RewardsPane {...props} />,
  },
] as const;
...
function ConfigurationNavigation({
  ...
  if (status === "created" && lobbyAddress) {
    lobbyContent = (
      <>
        ...
        <Row
          style={
            { height: "30px" } as CSSStyleDeclaration & React.CSSProperties
          }
        >
          <TextPreview
            style={{ margin: "auto" }}
  ...
  const creating =
    status === "creating" || (status === "created" && !lobbyAddress);

  return (
    <>
      <Title slot="title">Customize Lobby</Title>
      <div>
        Welcome Cadet! You can launch a copy of Dark Forest from this UI. We
        call this a Lobby.
        <Spacer height={12} />
        All settings will be defaulted to the same configuration of the main
        contract you are copying. However, you can change any of those settings
...
export function ConfigurationPane({
  ...
  const root = location.pathname.split("/settings")[0] || location.pathname;
  ...
  async function validateAndCreateLobby() {
    try {
      ...
      if (err instanceof InvalidConfigError) {
        setError(
          `Invalid ${err.key} value ${err.value ?? ""} - ${err.message}`
        );
      } else {
        setError(
          err?.message || "Something went wrong. Check your dev console."
        );
      }
    }
  }
