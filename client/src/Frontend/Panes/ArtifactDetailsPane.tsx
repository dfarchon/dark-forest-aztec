...
function ArtifactDescription({
  ...
  switch (artifact.artifactType) {
    case ArtifactType.BlackDomain:
      content = (
        <Text>
          When activated, permanently disables your planet. It'll still be
          yours, but you won't be able to do anything with it. It turns
          completely black too. Just ... gone. Because this one is{" "}
          <White>{rarityName}</White>, it works on planets up to level{" "}
          <White>{maxLevelBlackDomain}</White>. This artifact is consumed on
          activation.
        </Text>
      );
      break;
    case ArtifactType.BloomFilter:
      content = (
        <Text>
          When activated refills your planet's energy and silver to their
          respective maximum values. How it does this, we do not know. Because
          this one is <White>{rarityName}</White>, it works on planets up to
          level <White>{maxLevelBloomFilter}</White>. This artifact is consumed
          on activation.
        </Text>
      );
      break;
    case ArtifactType.Wormhole:
      content = (
        <Text>
          When activated, shortens the distance between this planet and another
          one. All moves between those two planets decay less energy, and
          complete faster.{" "}
          <Red>
            Energy sent through your wormhole to a planet you do not control
            does not arrive.
          </Red>{" "}
          Because this one is <White>{rarityName}</White>, it shrinks the
    ...
    case ArtifactType.PhotoidCannon:
      content = (
        <Text>
          Ahh, the Photoid Canon. Activate it, wait four hours. Because this one
          is <White>{rarityName}</White>, the next move you send will be able to
          go <White>{photoidRanges[artifact.rarity]}</White>x further and{" "}
          <White>{photoidSpeeds[artifact.rarity]}</White>x faster. During the 4
          hour waiting period, your planet's defense is temporarily decreased.
          This artifact is consumed once the canon is fired.
        </Text>
      );
      break;
    case ArtifactType.PlanetaryShield:
      content = (
        <Text>
          Activate the planetary shield to gain a defense bonus on your planet,
          at the expense of range and speed. When this artifact is deactivated,
          it is destroyed and your planet's stats are reverted--so use it
          wisely!
        </Text>
      );
      break;
