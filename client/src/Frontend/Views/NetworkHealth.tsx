import { AutoGasSetting, TooltipName } from "@dfpunk/types";
...
export function NetworkHealth() {
  ...
  const networkHealth = useEmitterValue(
    uiManager.getGameManager().networkHealth$,
    undefined
  );
        ...
        <Spread style={{ width: "100%" }}>
          {networkHealth &&
            Object.values(AutoGasSetting)
              .map((setting) =>
                networkHealth.find((entry) => entry[0] === setting)
              )
              .map(
                (entry) =>
                  entry && (
...
function NetworkHealthForGasSetting({
  ...
  return (
    <Sub>
      ...
      <Text>{(confirmationWaitTime / 1000).toFixed(2)}s</Text>
    </Sub>
  );
}
