...
import { ONE_DAY, weiToGwei } from "../../Backend/Utils/Utils";
...
export function TxConfirmPopup() {
  const params = useParams<TxConfirmParams>();
  ...
  const gasFee = BigInt(localStorage.getItem(`${account}-gasFeeGwei`) || "0");
      ...
      <div className="section">
        ...
        <Row className="mtop">
          <Checkbox
            ...
            onChange={(e: unknown) =>
              setAutoApprovedChecked(
                (e as { target: { checked: boolean } }).target?.checked ?? false
              )
            }
          />
        </Row>
