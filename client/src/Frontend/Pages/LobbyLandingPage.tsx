...
class LobbyPageTerminal {
  ...
  private async loadBalances(addresses: EthAddress[]) {
    ...
    const balances = await Promise.all(
      addresses.map((address) =>
        queue.add(() => this.ethConnection.loadBalance(address))
      )
    );
  ...
  public async chooseAccount() {
    this.terminal.printElement(
      <MythicLabelText text="                  Create a Lobby" />
    );
    this.terminal.newline();
    this.terminal.newline();
    this.terminal.printElement(
      <DarkForestTips tips={lobbyTips} title="Lobby Tips" />
    );
    this.terminal.newline();

    const accounts = getAccounts();
    this.terminal.println(
      `Found ${accounts.length} accounts on this device. Loading balances...`
    );
    this.terminal.newline();
        ...
        (account: EthAddress) => {
          if (connection) {
            terminal.current?.println(
              `Creating lobby with account: ${account}`
            );
            onReady(connection);
          } else {
