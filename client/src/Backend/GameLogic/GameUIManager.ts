...
import { biomeName, isLocatable, isSpaceShip } from "@dfpunk/gamelogic";
import { planetHasBonus } from "@dfpunk/hexgen";
...
class GameUIManager extends EventEmitter {
  ...
  /** Cached balance stream (bigint) mapped from GameManager; created on first getMyBalance$() call. */
  private balance$: Monomitter<bigint> | null = null;
  ...
  public getMyBalanceBn(): bigint {
    const balance = this.gameManager.getMyBalance();
    return typeof balance === "bigint" ? balance : BigInt(String(balance));
  }

  public getMyBalance$(): Monomitter<bigint> {
    if (!this.balance$) {
      this.balance$ = monomitter<bigint>(true);
      this.gameManager.getMyBalance$().subscribe((value) => {
        this.balance$!.publish(
          typeof value === "bigint" ? value : BigInt(String(value))
        );
      });
      this.balance$.publish(this.getMyBalanceBn());
    }
    return this.balance$;
  }
