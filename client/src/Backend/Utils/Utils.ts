import { formatNumber } from "@dfpunk/gamelogic";
import {
  EthAddress,
  Planet,
  SpaceType,
...
export function gweiToWei(gwei: number): bigint {
  return BigInt(Math.floor(gwei)) * GWEI;
}

export function weiToGwei(wei: bigint | { toString(): string }): number {
  const n = typeof wei === "bigint" ? wei : BigInt(wei.toString());
  return Number(n / GWEI);
}

export function weiToEth(wei: bigint): number {
  return Number(wei) / Number(ETH_WEI);
}

export function ethToWei(eth: number): bigint {
  return BigInt(Math.floor(eth * Number(ETH_WEI)));
}
