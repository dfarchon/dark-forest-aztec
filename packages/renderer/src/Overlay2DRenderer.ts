import { isEmojiFlagMessage } from "@dfpunk/gamelogic";
import {
  Artifact,
  CanvasCoords,
  Chunk,
...
export class Overlay2DRenderer {
  ...
  drawPlanetMessages(
    ...
    (
      renderInfo.planet.messages as PlanetMessage<unknown>[] | undefined
    )?.forEach((m) => {
  ...
  drawEmojiMessage(
    ...
    if (renderInfo.planet.emojiBobAnimation !== undefined) {
      const anim = renderInfo.planet.emojiBobAnimation as { value(): number };
      offsetY += anim.value() * (radiusPixels * 0.1);
    }
