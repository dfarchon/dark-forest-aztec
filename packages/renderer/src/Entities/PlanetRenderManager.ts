...
import { getOwnerColorVec, getPlanetCosmetic } from "@dfpunk/procedural";
    ...
    const txCol = (
      planet as Planet & {
        transactions?: {
          getTransactions(
            fn: unknown,
          ): import("@dfpunk/types").Transaction<
            import("@dfpunk/types").UnconfirmedMove
          >[];
        };
      }
    ).transactions;
    for (const unconfirmedMove of txCol?.getTransactions(isUnconfirmedMoveTx) ??
      []) {
