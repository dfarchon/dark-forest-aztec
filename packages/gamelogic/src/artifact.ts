...
const artifactCooldownHoursMap: Record<ArtifactType, number> = {
  ...
  [ArtifactType.ShipCrescent]: 0,
  [ArtifactType.ShipWhale]: 0,
  [ArtifactType.ShipGear]: 0,
  [ArtifactType.ShipTitan]: 0,
};

const artifactIsAncientMap: Map<ArtifactId, boolean> = new Map();

export function durationUntilArtifactAvailable(artifact: Artifact): number {
  return artifactAvailableTimestamp(artifact) - Date.now();
}

export function artifactAvailableTimestamp(artifact: Artifact): number {
  if (artifact.lastDeactivated === 0) {
    return Date.now();
  }
  const cooldownHours = artifactCooldownHoursMap[artifact.artifactType] ?? 24;
  return artifact.lastDeactivated * 1000 + cooldownHours * 60 * 60 * 1000;
}

export function isActivated(artifact: Artifact | undefined): boolean {
...
export const levelFromRarity = (rarity: ArtifactRarity): PlanetLevel => {
  ...
  if (rarity === ArtifactRarity.Legendary) return PlanetLevel.SEVEN;
  if (rarity === ArtifactRarity.Epic) return PlanetLevel.FIVE;
  if (rarity === ArtifactRarity.Rare) return PlanetLevel.THREE;
...
export function getPlayerControlledSpaceships(
  ...
  return (artifacts ?? []).filter(
    (a): a is Artifact => a?.controller === owner,
  );
}
