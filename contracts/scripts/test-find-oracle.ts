...
async function main() {
    // Address of the dedicated TestOracle system contract deployed via scripts/deploy.ts
    const oracleAddr = process.env.TEST_ORACLE_CONTRACT_ADDRESS;
    if (!oracleAddr) {
        throw new Error(
            'TEST_ORACLE_CONTRACT_ADDRESS not in .env — run pnpm deploy-contracts first.'
        );
    }
    ...
    const caller = await getOrCreateAccount(wallet, aztecNode);
    ...
    const chainId = await oracle.methods
        .get_chain_id()
    ...
    try {
        const noopTx = await oracle.methods.get_chain_id().send(sendOpts);
        await noopTx.wait();
        console.log('   Fresh block produced.');
    } catch {
        console.warn(
            '   No-op send failed (chain_id is view-only on some versions), continuing.'
        );
    }
    ...
    // NOTE: On current devnet versions the archive tree used by the oracle is
    // typically built from the earliest blocks and may not yet contain very
    // recent ones. To keep this script robust across runs we always query
    // block 1, which is guaranteed to be present in the archive as soon as the
    // chain has produced its first block.
    const targetBlock = 1;
    console.log(
        `\n--- Step 2: Target block for oracle query (fixed to ${targetBlock} for stable archive proofs) ---`
    );
    ...
    if (events.length === 0) {
        console.warn(
            '   No BlockHashResult events found (may need to wait more blocks).'
        );
    } else {
    ...
    if (events.length > 0) {
        ...
        console.log(
            ...
            hashMatch
                ? 'PASS'
                : `FAIL (event=${eventHash}, storage=${storageHash})`
        );
    }
